...
当前 main 源码含 SupplySync `1.1.19`，套装仍为 `0.1.0-dev`，不是稳定版。本批不新增 tag／Release，旧 `v0.1.0-preview.1` 固定附件不含本次诊断更新。公开源码为 [aiiqc/acg-faka-extensions](https://github.com/aiiqc/acg-faka-extensions)；已有下载附件及 SHA256 以[预发布页](https://github.com/aiiqc/acg-faka-extensions/releases/tag/v0.1.0-preview.1)实际内容为准，不把 main 自动源码包当成已验证新发行包。本批相关 PHP 行为及原生安装恢复的隔离证据与未覆盖边界见[项目状态](PROJECT_STATUS.md)。第 3 节缺少已核参数时会停止；源码或预览版不是生产许可。媒体权利与安全边界继续见[安全说明](../SECURITY.md)。

当前源码对应 PikaCatalogHub `0.6.9`／PikaSupplySync `1.1.19`、主题 `1.1.7`、支付适配器 `0.1.2`。[安全诊断](../extensions/PikaSupplySync/Wiki/README.md#本轮安全诊断)与[原生 basic 手动空库存保护](../extensions/PikaSupplySync/Wiki/README.md#手动商品库存未知保护)复用同一同步链。1.1.19 与 1.1.18 的 StateStore 相同，回退到 1.1.18 不需删除库存保护计数；只有回到 1.1.17 或更早时才按后者说明受控投影。最终制品、目标站部署和真实交易仍须分别验收，不能沿用旧版或其他站点结论。BEpusdt 后端单独部署，不包含在扩展安装器内。本文路径、账号和站点命名均为通用示例，不包含实际凭据。第一次使用先读[新手操作路径](USER_GUIDE.md)，再按本教程执行安装、预检和恢复。
...
1.1.19 可展开本轮诊断，分别查看失败商品数、诊断事件总数与最多 20 条保留事件。`error_total` 不等于 `failed`，一件商品可能有多条事件；截断或旧记录缺少明细时，不从总数推断原因。商品仅用编号哈希定位，货源级错误没有商品哈希。上游固定原因均冠以“上游返回的声明，未核实根因”；HTTP 200 不代表业务成功，业务码和响应数据形状须分开看。

请求计时是从该次 curl 开始累计到 DNS、连接、TLS、首字节和结束的里程碑，不能相加或理解为独立阶段耗时；DNS 值不含连接策略在 curl 外部执行的查询。接收字节仅指响应正文，不含请求头或响应头。最近请求／最近失败及最多三次尝试不是均值、p95、完整历史或全商品覆盖率；缺失值表示未取得，不补成 0。页面不展示上游原文、地址或凭据，也不会自动重跑。
