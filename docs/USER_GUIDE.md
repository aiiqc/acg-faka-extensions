...
对应 Hub 0.6.9／SupplySync 1.1.17，并补充 main 中 Pika 1.1.7 的店名修复与演示；旧 `v0.1.0-preview.1` 固定附件仍为主题 1.1.6，不含本修复。本导览不是生产验收凭证。固定下载身份以[预发布页](https://github.com/aiiqc/acg-faka-extensions/releases/tag/v0.1.0-preview.1)实际附件为准；缺失时停止。先按顺序看懂流程，实际安装与服务器命令请用[安装教程](USER_INSTALL.md)。截图由各节说明的页面代码配合合成数据生成，不包含实际商户、客户、密钥或支付地址。
...
main 源码中的 Pika 1.1.7 支持完整店名换行；旧预发布下载附件仍为 1.1.6，不包含这次修复。

修改公告外部链接前，先在「网站设置 → 安全设置 → 外链域名白名单」保存需要的域名，保持过滤开启，再回「基本设置 → 店铺公告」通过 HTML 源码模式恢复内容。允许名单作用于全站，填写父域名也会允许子域名；请只允许必要域名。已被移除的链接地址不会自动恢复。**即使只改店名，保存基本设置也会重新提交公告**；源码模式不会绕过安全过滤。完整顺序见[店名与公告外部链接](USER_INSTALL.md#851-店名与公告外部链接)。

![Pika 1.1.7 前台主题合成演示](images/pika-theme-117-desktop.png)

实际运行的主题模板、CSS 和 JS，使用虚构店名、商品与价格及中性素材；不含真实货源或交易。另见[375px 手机宽度演示](images/pika-theme-117-mobile.png)，它不是手机实机验收。
...
| Pika 前台、会员页、订单等待 | Pika 1.1.7 前台桌面／375px 合成演示已提供，使用中性素材；会员页和订单等待仍缺安全截图，不以生产截图替代 |
